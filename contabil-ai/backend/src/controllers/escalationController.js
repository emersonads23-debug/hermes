...
async function list(req, res) {
  try {
    ...
    res.status(500).json({ error: 'Erro ao listar escalacoes' });
  }
}
...
async function update(req, res) {
  try {
    ...
    logger.error('Failed to update escalation', { error: err.message });
    res.status(500).json({ error: 'Erro ao atualizar escalacao' });
  }
}

...
async function storeMemory(companyId, memoryType, memoryKey, memoryValue, { source = 'system', confidence = 0.5 } = {}) {
  try {
    ...
    logger.error('storeMemory failed', { companyId, memoryType, memoryKey, error: err.message });
    throw err;
  }
}
...
async function updateMemory(memoryId, { memoryValue, confidence } = {}) {
  try {
    ...
    logger.error('updateMemory failed', { memoryId, error: err.message });
    return null;
  }
}
...
async function getMemory(companyId, memoryType, memoryKey) {
  try {
    ...
    logger.error('getMemory failed', { companyId, memoryType, memoryKey, error: err.message });
    return null;
  }
}
...
async function searchMemory(companyId, searchKey, options = {}) {
  try {
    ...
    logger.error('searchMemory failed', { companyId, searchKey, error: err.message });
    return [];
  }
}
...
async function getRelevantMemories(companyId, { memoryType, minConfidence = 0.8, limit = 50 } = {}) {
  try {
    ...
    logger.error('getRelevantMemories failed', { companyId, memoryType, error: err.message });
    return [];
  }
}
...
async function lookupSupplierCategory(companyId, supplierName) {
  try {
    ...
    logger.error('lookupSupplierCategory failed', { companyId, supplierName, error: err.message });
    return null;
  }
}
...
async function lookupAccountMapping(companyId, description) {
  try {
    ...
    logger.error('lookupAccountMapping failed', { companyId, description, error: err.message });
    return null;
  }
}
...
async function learnFromClassification(companyId, { supplier, classification, accountCode, category, source = 'classification_confirmed' }) {
  try {
    ...
    logger.error('learnFromClassification failed', { companyId, error: err.message });
  }
}
...
async function learnFromReconciliation(companyId, { description, category, accountCode, amount, source = 'reconciliation_confirmed' }) {
  try {
    ...
    logger.error('learnFromReconciliation failed', { companyId, error: err.message });
  }
}
...
async function learnFromTransactionPattern(companyId, { patternKey, patternType, patternData, occurrences, source = 'pattern_detection' }) {
  try {
    ...
    logger.error('learnFromTransactionPattern failed', { companyId, patternKey, error: err.message });
    return null;
  }
}
...
async function learnFinancialBehavior(companyId, { behaviorKey, behaviorData, source = 'analysis' }) {
  try {
    ...
    logger.error('learnFinancialBehavior failed', { companyId, behaviorKey, error: err.message });
  }
}
...
async function buildMemoryContext(companyId) {
  try {
    ...
    logger.error('buildMemoryContext failed', { companyId, error: err.message });
    return '';
  }
}
